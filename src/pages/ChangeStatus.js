...
import test from "../assets/change.jpg";
...
export const ChangeStatus = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { oldname, name, age, amount } = formData;
    setpersonname(name);
    setpersonage(age);
    setpersonamount(amount);
    try {
      if (window.ethereum) {
        const tx = await contract.changePeople(oldname, name, age, amount);
        await tx.wait();
    ...
    <div className="bg-gray-900 animate-gradient shadow overflow-hidden ">
      <form onSubmit={handleSubmit} className="max-w-md mx-auto mt-8 ">
        ...
        <div>
          <label
            htmlFor="account"
            className="block text-gray-100 font-bold my-2"
          >
            Your Account Name
          </label>
          <input
